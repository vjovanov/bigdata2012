...
module for printing. In trait \code{PrintlnExp} we define how the \code{println}
operation is linked to the intermediate representation. The \code{reflectEffect}
method defines that the \code{profile} method has global side effects which
...
\code{PrintlnGen} trait, we define how the code for \code{println} is generated
for Scala.
...
All the above mentioned algorithms are repeated for program scopes at one level
...
how CSE extracts \code{size} and inlining replaces \code{parse} and \code{split}
invocations with their bodies. In step \ref{lst:aos-soa}) the array \code{x} of
