...
In Section \ref{sec:programming-model} we have shown that the \code{DList}
...
of monadic operations \code{map}, \code{flatMap} and \code{filter} through the
...
the existing mechanism to the \code{DList} operations although they are not
...
\cite{coutts_stream_2007} approach of converting \code{DList} to streams and
...
preceding \code{DList}. $shape\_dep$ replaces the shape variable (e.g.
...
\code{DList}s. This would be possible for Hadoop, as it supports multiple
outputs in the \code{Map} phase but is not currently supported in Spark, Scoobi
