...
\scode{DList[T]}. \scode{DList[T]} represents a distributed collection of
elements that have type \scode{S} which is a subtype of \scode{T}. The elements
of a \scode{DList[T]} collection are immutable, so each operation on the list
can only: \emph{i)} produce a new \scode{DList}, \emph{ii)} save it to
...
\code{DList} operations are presented in Table \ref{tbl:operations}. In the left
column, we show which frameworks support which method. The middle column shows
the method name. Finally, the right column contains the type of \code{DList}
...
Operations \code{DList()} and \code{save} are used for loading and storing data
...
and can also be used with Scala \code{for} comprehensions. Operations
\code{groupByKey}, \code{join}, \code{cogroup}, \code{cross} and \code{reduce}
are applicable only if the elements of \code{DList} form a key/value tuple.
...
\code{sort} sorts the dataset, \code{partitionBy} defines partitioning among
machines, and \code{cache} signals that data should be kept in cluster memory
for faster future accesses. Two \code{DList}s can be concatenated by the
\code{++} operation. A DList can be materialized on the master node by calling
\code{materialize()}.
...
example, in which the code does not have any visible \code{Rep} types. Since a
