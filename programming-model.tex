...
\multirow{2}{*}{Spark}			        & \scode{cache()} 								& \scode{DColl[T] => DColl[T]}\\ %\cline{2-3} 
								& \scode{sort(cmp: Rep[Comparator[T]])}			& \scode{DColl[T] => DColl[T]}\\ \hline
Crunch						        & \scode{sort(asc: Rep[Boolean])}					& \scode{DColl[T] => DColl[T]} \\ \hline
\end{tabular}
...
are applicable only if the elements of \code{DColl} form a key/value tuple.
...
for faster future accesses. Two \code{DColl}s can be concatenated by the
